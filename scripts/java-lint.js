...
const javaFilesCommand = 'find ./android -name "*.java" | grep -v generated';

function download(url, downloadPath, resolve, reject, redirectCount) {
  https.get(url, (response) => {
    switch (response.statusCode) {
      case 302: {
        ...
        if (redirectCount === 0) {
          throw new Error(
            `Unhandled response code (HTTP${response.statusCode})` +
              `while retrieving google-java-format binary from ${url}`,
          );
        }

        download(response.headers.location, downloadPath, resolve, reject, redirectCount - 1);
        break;
      }
      case 200: {
        // OK
...
function downloadP(url, downloadPath) {
  return new Promise((resolve, reject) => {
    download(url, downloadPath, resolve, reject, 1);
  });
}

function filesWithLintingIssues() {
  const proc = exec(`java -jar ${googleJavaFormatPath} --dry-run $(${javaFilesCommand})`, {
    silent: true,
  });
...
function unifiedDiff(file) {
  const lintedProc = exec(`java -jar ${googleJavaFormatPath} --set-exit-if-changed ${file}`, {
    silent: true,
  });
...
async function main() {
  ...
  await downloadP(googleJavaFormatUrl, googleJavaFormatPath);
